...
/// Strategy for handling map-like schemas with enum keys.
#[derive(Clone, Debug)]
pub enum MapSchemaMode {
    /// Preserve enum-keyed properties as generated by schemars.
    Preserve,
    /// Convert enum-keyed properties into additionalProperties maps.
    /// This broadens the allowed key set and may admit keys not present in the original schema.
    AdditionalProperties,
}

impl Default for MapSchemaMode {
    fn default() -> Self {
        Self::Preserve
    }
}

/// Apply the selected map schema strategy to a JSON Schema.
pub fn apply_map_schema_mode(schema: &mut Value, mode: MapSchemaMode) {
    if matches!(mode, MapSchemaMode::Preserve) {
        return;
    }
    transform_enum_keyed_maps(schema);
}

fn transform_enum_keyed_maps(schema: &mut Value) {
    match schema {
        Value::Object(map) => {
            for value in map.values_mut() {
                transform_enum_keyed_maps(value);
            }

            if let Some(first_schema) = uniform_properties_schema(map) {
                map.insert("type".to_string(), Value::String("object".to_string()));
                map.insert("additionalProperties".to_string(), first_schema);
...
fn uniform_properties_schema(map: &Map<String, Value>) -> Option<Value> {
    let props = match map.get("properties").and_then(|v| v.as_object()) {
        Some(props) if props.len() >= 2 => props,
        _ => return None,
    };
